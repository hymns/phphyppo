 ...
 */
class Apps_Model extends AppModel
{
	/**
	 * show_table
	 *
	...
	public function desc_table($tablename)
	{
		// pgsql database
		if ($this->db->db_type == 'pgsql')
			$this->db->query('SELECT f.attname AS field, f.attnotnull AS null, pg_catalog.format_type(f.atttypid, f.atttypmod) AS type, CASE WHEN p.contype = \'p\' THEN \'true\' ELSE \'false\' END AS primary, CASE WHEN f.atthasdef = \'t\' THEN d.adsrc END AS extra FROM pg_attribute f JOIN pg_class c ON c.oid = f.attrelid JOIN pg_type t ON t.oid = f.atttypid LEFT JOIN pg_attrdef d ON d.adrelid = c.oid AND d.adnum = f.attnum LEFT JOIN pg_namespace n ON n.oid = c.relnamespace LEFT JOIN pg_constraint p ON p.conrelid = c.oid AND f.attnum = ANY ( p.conkey ) LEFT JOIN pg_class AS g ON p.confrelid = g.oid WHERE c.relkind = \'r\'::char AND n.nspname = \'public\' AND c.relname = \'' . $this->db->db_prefix . $tablename . '\' AND f.attnum > 0 ORDER BY f.attnum');
		
		// mysql database
		else
			$this->db->query('DESC ' . $this->db->db_prefix . $tablename);
